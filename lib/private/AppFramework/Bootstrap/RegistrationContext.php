...
use OCP\Profile\IActionManager as IProfileActionManager;
...
class RegistrationContext {
	...
	/** @var ServiceRegistration<IProfileAction>[] */
	...
	/**
	 * @psalm-param class-string<IProfileAction> $capability
	 */
	 ...
	 * @param App[] $apps
	 */
	public function delegateProfileActionRegistrations(array $apps, IProfileActionManager $profileActionsManager): void {
		while (($action = array_shift($this->profileActions)) !== null) {
			try {
				$profileActionsManager->queueAction($action->getService());
			} catch (Throwable $e) {
				$appId = $action->getAppId();
				$this->logger->error("Error during profile action registration of $appId: " . $e->getMessage(), [
					'exception' => $e,
				]);
			}
		}
